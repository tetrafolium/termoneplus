...
class TerminalEmulator {
   ...
   */
  @EscapeProcessingState private int mEscapeState;
   ...
   */
  public TerminalEmulator(TermSession session, TranscriptScreen screen,
                          int columns, int rows, ColorScheme scheme) {
    mSession = session;
    mMainBuffer = screen;
    mScreen = mMainBuffer;
  ...
  private void process(byte b) { process(b, true); }
  ...
  private void finishSequence() { mEscapeState = EscapeProcessingState.NONE; }
  ...
  private void doEscSelectLeftParen(byte b) { doSelectCharSet(0, b); }

  private void doEscSelectRightParen(byte b) { doSelectCharSet(1, b); }
  ...
  private int getDefaultStyle() {
    return TextStyle.encode(mDefaultForeColor, mDefaultBackColor,
                            TextStyle.fxNormal);
  }
   ...
   * @param style The encoded text style (effects, foreground and background
   *     colors)
   */
  ...
  private void emit(int c) { emit(c, getStyle()); }
  ...
  @IntDef({EscapeProcessingState.NONE, EscapeProcessingState.START,
           EscapeProcessingState.POUND, EscapeProcessingState.SELECT_LEFT_PAREN,
  ...
  private @interface EscapeProcessingState {
    ...
    int RIGHT_SQUARE_BRACKET_ESC =
        9; // ESC ] (AKA OSC - Operating System Controls)
  }
}
