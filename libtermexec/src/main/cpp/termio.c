...
static void termios_setUTF8Input(JNIEnv *env, jobject clazz, jint fd,
                                 jboolean flag) {
  struct termios tio;
  ...
  if (tcgetattr(fd, &tio) != 0) {
    throwIOException(
        env, "Failed to get the parameters associated with the terminal");
    return;
  }
  ...
  if (tcsetattr(fd, TCSANOW, &tio) != 0) {
    throwIOException(
        env, "Failed to set the parameters associated with the terminal");
  }
}

static void ioctl_setWindowSize(JNIEnv *env, jobject clazz, jint fd, jint row,
  ...
  if (ioctl(fd, TIOCSWINSZ, &arg) != 0) {
    ...
    snprintf(msg, sizeof(msg),
             "ioctl: set tty window size fail with error code %d", errno);
...
int register_termio(JNIEnv *env) {
  ...
  return register_native(env, "com/termoneplus/TermIO$Native", methods,
