...
private static final String ACTION_OPEN_NEW_WINDOW =
	"com.termoneplus.OPEN_NEW_WINDOW";
private static final String ACTION_RUN_SCRIPT = "com.termoneplus.RUN_SCRIPT";

private static final String ARGUMENT_WINDOW_HANDLE =
	"com.termoneplus.WindowHandle";
private static final String ARGUMENT_SHELL_COMMAND =
	"com.termoneplus.Command";

private static final String PERMISSION_RUN_SCRIPT =
	BuildConfig.TERM_APPLICATION_ID + ".permission.RUN_SCRIPT";
...
private void errorActivityNotFound(View view) {
	Toast toast =
		Toast.makeText(view.getContext(), R.string.error_activity_not_found,
		               Toast.LENGTH_LONG);
...
private void errorPermissionDenial(View view) {
	if (android.os.Build.VERSION.SDK_INT <
	    android.os.Build.VERSION_CODES.M /*API level 23*/) {
		Toast toast =
			Toast.makeText(view.getContext(), R.string.error_security_reinstall,
			               Toast.LENGTH_LONG);
		toast.setGravity(Gravity.BOTTOM | Gravity.START | Gravity.LEFT, 0, 0);
		toast.show();
	} else {
		...
		Snackbar
		.make(main_layout, R.string.error_security_grant,
		      Snackbar.LENGTH_INDEFINITE)
		.setAction(android.R.string.yes, v->permissionRunScript())
		.show();
	}
}
...
private void permissionRunScript() {
	...
	if (ActivityCompat.shouldShowRequestPermissionRationale(
		...
		Snackbar
		.make(main_layout, R.string.run_script_rationale,
		      Snackbar.LENGTH_INDEFINITE)
		.setAction(android.R.string.yes, v->requestPermissionRunScript())
		.show();
	} else {
